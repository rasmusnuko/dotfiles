...
alias vultr="ssh root@70.34.221.208"
...
mp3p () {
  yt-dlp --ignore-errors --sleep-interval 30 -i -f bestaudio --extract-audio --audio-format wav --audio-quality 0 -o '%(playlist)s/%(title)s.%(ext)s' "$1"
}

mp3proxy (){
    yt-dlp --proxy $2 --ignore-errors --sleep-interval 30 -i -f bestaudio --extract-audio --audio-format mp3 --audio-quality 0 -o '%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s' "$1"
}
